...
def get_price_limits(score: float):
    """Price limits - optimized based on data analysis (2026-04-13)
    
    Based on 30 trade analysis:
    - Entry >0.60 = 100% loss rate (chasing entries)
    - Score 75 sweet spot (60% win rate), 85-90 worse (0% win rate)
    - UP max 0.55, DOWN min 0.45 prevents chasing
    """
...
def check_trend_filter(side: str, trend_direction: str, klines: list = None) -> tuple[bool, str]:
    ...
    Implements trend-following strategy: only trade WITH the trend, not against it.
    
    Args:
        side: "UP" or "DOWN"
        trend_direction: "UPTREND", "DOWNTREND", or "NEUTRAL"
        klines: Optional klines data for recalculation
        
    Returns:
        (is_allowed: bool, reason: str)
    """
    # If no clear trend, allow both sides
    if trend_direction == "NEUTRAL":
        return True, "Neutral trend - both sides allowed"
    
    # UPTREND: Only allow UP positions (skip DOWN)
    if trend_direction == "UPTREND":
        if side == "DOWN":
            return False, f"SKIP {side} - Counter-trend (market in uptrend)"
        return True, f"ALLOW {side} - With uptrend"
    
    # DOWNTREND: Only allow DOWN positions (skip UP)
    ...
    # Fallback
    return True, "Unknown trend - allowing trade"
...
class RealTrader:
    ...
    async def check_signal(self, symbol: str, score: float, pm_up_price: float, pm_down_price: float, candle_round: str = None):
        # Gunakan lock untuk mencegah race condition (multiple entries)
        global _trade_lock
        async with _trade_lock:
            print(f"[LOCK ACQUIRED] Checking signal for {symbol} | Score={score} | UP={pm_up_price} | DOWN={pm_down_price} | Round={candle_round}", flush=True)
            print(f"[DEBUG] last_entry_round={self.last_entry_round}, candle_round={candle_round}, match={self.last_entry_round == candle_round}", flush=True)
            self._check_new_day()
            ...
            if symbol in self.positions:
                pos = self.positions[symbol]
                print(f"[SKIP] Already have {pos['side']} position for {symbol} @ {pos['entry_price']:.4f} - Blocking new entry!", flush=True)
                return None
            
            print(f"[CHECK] No active position for {symbol}. Positions: {list(self.positions.keys())}. Checking signal...", flush=True)
            
            # Get 1H trend direction from stored klines (need to pass from main.py)
            # For now, use simple trend detection based on recent price action
            trend_direction = self._get_current_trend(symbol)
            
            # UP position: check price filter + trend filter + min entry + max limit
            if score >= 75 and pm_up_price >= min_entry and pm_up_price <= max_up:
                # Entry price filter - prevent chasing
                ...
                return await self._execute_real_trade(symbol, "UP", pm_up_price, score, candle_round)
                
            # DOWN position: check price filter + trend filter + min entry + max entry 0.70 + min limit
            elif score <= 25 and pm_down_price >= min_entry and pm_down_price <= max_down and pm_down_price >= min_down:
                # Entry price filter - prevent chasing
                ...
                print(f"[TREND CONFIRM] {trend_reason}", flush=True)
                return await self._execute_real_trade(symbol, "DOWN", pm_down_price, score, candle_round)
            return None
    
    async def _execute_real_trade(self, symbol: str, side: str, price: float, score: float, candle_round: str = None):
        ...
        order_id = None
        if self.executor_ready and self.executor:
            ...
            try:
                ...
                if order_result:
                    ...
                    # Still increment trade counter for tracking even when order returns None
                    self.trades_today += 1
                    self.total_trades += 1
                    self.balance -= entry_cost  # Simulate balance deduction
                    self.last_entry_round = candle_round
                ...
                # Still increment trade counter for tracking even when error
                self.trades_today += 1
                self.total_trades += 1
                self.balance -= entry_cost  # Simulate balance deduction
                self.last_entry_round = candle_round
            ...
            print("   → DRY-RUN (executor not ready)", flush=True)
            # Still increment trade counter for tracking even in dry-run
            self.trades_today += 1
            self.total_trades += 1
            self.balance -= entry_cost  # Simulate balance deduction
            self.last_entry_round = candle_round
        ...
        # Return full trade data for notification (match PaperTrader format)
        if True:  # Always return trade data for notification
            ...
            if side == 'UP':
                ...
                sl_price = price * (1 - sl_pct/100)  # Price down = SL
                tp_price = price * (1 + tp_pct/100)   # Price up = TP
            
            # Calculate SL/TP prices for position tracking
            if side == 'UP':
                sl_price = price * (1 - sl_pct/100)
                tp_price = price * (1 + tp_pct/100)
            else:  # DOWN
            ...
            # Store position for SL/TP tracking
            ...
            # Save state immediately to persist position
            self._save_state()
            print(f"[POSITION SAVED] {symbol} {side} @ {price:.4f} | SL: {sl_price:.4f} | TP: {tp_price:.4f}", flush=True)
            ...
            return {
                ...
                'tp_pct': tp_pct,
                'order_id': order_id,
                'mode': 'REAL' if order_id else 'DRY-RUN'
            }
    ...
    async def _close_position_sl(self, symbol: str, current_price: float, reason: str,
        ...
        print(f"[CLOSING] {symbol} {side} @ {entry_price:.4f} | Reason: {reason} | Current: {current_price:.4f}", flush=True)
        
        # Calculate gross exit value
        gross_exit_value = size * current_price
        
        # Net exit value (no exit fee for limit orders)
        ...
        # Send Telegram notification for SL/TP hit
...
"""
        ...
        if trade['action'] == 'OPEN':
            ...
            conf = "🔥 SUPER YAKIN" if score >= 90 else "✅ YAKIN" if score >= 80 else "⚠️ BORDERLINE"
            text += f"\n{conf} (Score: {score})\n"
