    ...
    while True:
        try:
            if state.mid > 0 and state.klines:
                # Calculate trend score for notifications
                ...
                # Calculate candle round for entry tracking (always needed)
                ...
                if direction == "NEUTRAL" and dash_state.should_notify_neutral(coin, tf) and TELEGRAM_ENABLED:
                    try:
                        ...
                        if hasattr(paper_trader, 'positions') and symbol_key in paper_trader.positions:
                            ...
                            current_pnl = 0
                            if pos_side == 'UP':
                                current_pnl = (state.pm_up or 0 - pos_price) * pos.get('size', 0)
                            else:  # DOWN
                                current_pnl = (pos_price - state.pm_dn or 0) * pos.get('size', 0)
                            pnl_emoji = "🟢" if current_pnl >= 0 else "🔴"
                            position_info = f"\n📍 <b>POSITION ACTIVE</b>\n{pos_side} @ {pos_price:.4f} | {pnl_emoji} PnL: ${current_pnl:+.2f}\n🛑 SL: {pos_sl:.4f} | 🎯 TP: {pos_tp:.4f}\n"
                ...
                # STEP 1: EXECUTE TRADE FIRST (silent execution)
                ...
                # STEP 2: CHECK SL/TP FOR EXISTING POSITIONS (every cycle)
                try:
                    if hasattr(paper_trader, 'check_sl_tp'):
                        sltp_trade = await paper_trader.check_sl_tp(f"{coin}-{tf}", state.pm_up or 0, state.pm_dn or 0)
                    else:
                        sltp_trade = None
                    ...
                    import traceback
                    print(f"[ERROR] SL/TP check TypeError: {e}", flush=True)
                    print(f"[ERROR] Traceback: {traceback.format_exc()}", flush=True)
                except Exception as e:
                    print(f"[ERROR] SL/TP check failed: {e}", flush=True)
                
                # STEP 3: NOTIFY BOTH SIGNAL AND TRADE (only if trade actually happened with data)
                if trade_executed and trade and TELEGRAM_ENABLED:
                    try:
                        # First: Signal Entry notification
                        emoji = "🚀" if score >= 75 else "🔻"
                        conf_text = "🔥 SUPER YAKIN" if score >= 90 or score <= 10 else "✅ YAKIN" if score >= 80 or score <= 20 else "⚠️ BORDERLINE"
...
⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ET"""
                        ...
                        # Second: Detailed Trade notification (only if trade has valid data)
                        if trade and trade.get('side') and trade.get('price'):
                            # Try format_trade_alert first, fallback to manual format
                            try:
                                if hasattr(paper_trader, 'format_trade_alert'):
                                    alert_text = paper_trader.format_trade_alert(trade)
                                else:
                                    # Manual format if method not available
                                    emoji = "🟢" if trade['side'] == 'UP' else "🔴"
                                    sl_pct = trade.get('sl_pct', 20)
                                    tp_pct = trade.get('tp_pct', 40)
                                    entry = trade['price']
                                    if trade['side'] == 'UP':
                                    ...
                                    conf = "🔥 SUPER YAKIN" if trade.get('score',75) >= 90 else "✅ YAKIN" if trade.get('score',75) >= 80 else "⚠️ BORDERLINE"
                                    
                                    alert_text = f"""{emoji} <b>PAPER TRADE - OPEN</b> {emoji}
...
⏰ {trade['timestamp'][:19]} ET"""
                        ...
                        print(f"[NOTIFY SENT] Signal + Trade for {direction} {score}", flush=True)
                    except Exception as e:
                ...
                try:
                    ...
                    status = paper_trader.get_status()
                    print(f"[{now}] BTC={state.mid:,.0f} | Score={score:.0f} | {direction} | Trades={status['total_trades']} | PnL=${status['total_pnl']:.2f}", flush=True)
                except Exception as e:
